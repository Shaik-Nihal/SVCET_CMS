...
<div class="sidebar" id="sidebar">
  ...
  <nav class="nav flex-column">
    ...
    <a class="nav-link active" href="<?= APP_URL ?>/staff/notifications.php"><i class="bi bi-bell-fill"></i>Notifications<?php if ($unreadCount): ?><span class="badge bg-danger ms-auto"><?= $unreadCount ?></span><?php endif; ?></a>
    ...
    <div class="card-body p-0">
      ...
      <div class="p-3 d-flex align-items-start gap-3 border-bottom <?= !$n['is_read'] ? 'bg-light' : '' ?>"
        ...
        <div class="flex-grow-1">
          <?php if ($n['ticket_id']): ?>
          <a href="<?= APP_URL ?>/staff/ticket_detail.php?id=<?= $n['ticket_id'] ?>" class="text-decoration-none text-dark"><?= h($n['message']) ?></a>
          <?php else: ?>
